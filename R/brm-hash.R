...
#==>FILE_SEPARATOR
#filename:SomeNEwChecks.R
#folder:


a <- brm( count ~  zAge + zBase * Trt + (1|patient),
          data = epilepsy, family = poisson() , file = "aa" )

b <- brm( count ~  zAge + zBase * Trt + (1|patient),
          data = epilepsy, family = poisson() , file = "aa" )


d<- brm( count ~  zBase * Trt + (1|patient),
         data = epilepsy, family = poisson() , file = "aab"  )


f <- count ~  zAge + zBase * Trt + (1|patient)
...
fam2 = gaussian()
fam2a = gaussian()
rec_try <- function(liste){
  if( !is.list(liste)){
    cat(".")
    cat("\n")
    return(NULL )
  }
  for (prop in names(liste)){
    cat( prop )
    cat("\n")
    # print( liste[[prop]] )
    # cat("\n")
...
a <- remove_env_attrs( fam2 )
a2 <- remove_env_attrs( fam2a )
...
f1 <- count ~  zAge + zBase * Trt + (1|patient)
f2 <- count ~  zAge + zBase * Trt + (1|patient)
f1a <- remove_env_attrs( f1 )
f2a <- remove_env_attrs( f2  )
...
# Complex formula with functions and multiple environments
test_that("removes environments in complex formula with functions", {
  env1 <- new.env()
  env2 <- new.env()
  f <- as.formula(~ s(x) + y:z, env = env1)
  environment(f) <- env2
  cleaned <- remove_env_attrs(f)
  expect_null(attr(cleaned, ".Environment"))
  expect_null(environment(cleaned))
})

# List of formulas with different environments
test_that("removes environments in list of formulas", {
  f1 <- y ~ x
  f2 <- a ~ b
  attr(f1, ".Environment") <- new.env()
  environment(f2) <- new.env()
  flist <- list(f1, f2)
  cleaned <- remove_env_attrs(flist)
  expect_null(attr(cleaned[[1]], ".Environment"))
  expect_null(environment(cleaned[[2]]))
})

# Family inside a list, with environment in an argument
test_that("removes environment from family inside a list", {
  fam <- gaussian(link = "identity")
  attr(fam, ".Environment") <- new.env()
  l <- list(family = fam)
  cleaned <- remove_env_attrs(l)
  expect_null(attr(cleaned$family, ".Environment"))
})

# Nested list with formulas and families
test_that("removes environments from deeply nested structures", {
  env <- new.env()
  f <- y ~ poly(x, 2)
  environment(f) <- env
  fam <- poisson()
  attr(fam, ".Environment") <- env
  nested <- list(
    level1 = list(
      level2 = list(
        formula = f, family = fam
      )
    )
  )
  cleaned <- remove_env_attrs(nested)
  expect_null(environment(cleaned$level1$level2$formula))
  expect_null(attr(cleaned$level1$level2$family, ".Environment"))
})

# A call object with environment attribute
test_that("removes environment from call objects", {
  c <- quote(a + b)
  attr(c, ".Environment") <- new.env()
  cleaned <- remove_env_attrs(c)
  expect_null(attr(cleaned, ".Environment"))
})

# Formula embedded as a component in a custom S3 object
test_that("removes environment from formula in custom object", {
  my_obj <- list(data = 1:5, f = y ~ x)
  class(my_obj) <- "myclass"
  environment(my_obj$f) <- new.env()
  cleaned <- remove_env_attrs(my_obj)
  expect_null(environment(cleaned$f))
})

# List with mixed types: formula, family, numeric, call
test_that("removes environments from mixed type list", {
  f <- y ~ x
  environment(f) <- new.env()
  fam <- binomial()
  attr(fam, ".Environment") <- new.env()
  call_obj <- quote(sum(a, b))
  attr(call_obj, ".Environment") <- new.env()
  l <- list(f = f, fam = fam, n = 42, call = call_obj)
  cleaned <- remove_env_attrs(l)
  expect_null(environment(cleaned$f))
  expect_null(attr(cleaned$fam, ".Environment"))
  expect_null(attr(cleaned$call, ".Environment"))
  expect_equal(cleaned$n, 42)
})
